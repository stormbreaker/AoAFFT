...
* 	and then it calcluates the Fast Fourier Transform of both the original
* 	polynomial and some shifts of it in a "sliding window" pattern.
* 	It then prints the top five peaks and the indices where they were
* 	found.     
******************************************************************************/
using namespace std;
...
*	I add after finding max values. I again modified it and commented out
*	the modified Logar Algorithm however I did not delete it.   
* Parameters:
...
/*	double* copy1  = new double[n];
	...
	int index = 0;
		...
		{
			if (copy1[j]> copy1[j - 1] && maxes[index] > copy1[j+1])
			{
				maxes[index] = copy1[j];
			}
	...
	for (int i = 0; i < 5; i++)
	{
		...
		for (int j = 2; j < n/2; j++)
		{
			if (a[j] > a[j-1] && a[j] > a[j+1])
			{
				...
				{
					maxval = a[j];
					maxval = maxval + a[j-2] + a[j-1] + a[j+1] + a[j+2];
					tempindex = j;
				}
			}
		}
...
*	It also outputs the peaks to the file.  
* Parameters:
